...
from domain.providers import GROQ_DEFAULT_MODEL, GROQ_REASONING_EFFORT, GROQ_CREDENTIAL_REFERENCE, GROQ_CAPABILITIES

GROQ_VISUAL_MODEL = GROQ_DEFAULT_MODEL
...
class GroqVisualQualityProvider(VisualQualityProvider, RuntimeOnly):
    """Groq qwen/qwen3.8-27b 視覺品質審查提供者。"""

    def __init__(self, api_key, model=None, *, client=None):
        self.__api_key = api_key
        self.__model = model or GROQ_VISUAL_MODEL
        self.__temperature = GROQ_TEMPERATURE
        self.__max_tokens = GROQ_MAX_TOKENS
...
{{
  ...
  "reviewed_viewports": ["desktop", "mobile"]
    ...
    def _parse_response(self, raw: str) -> VisualReviewResult:
        """解析模型回應 - strict fail-closed parsing."""
        try:
            cleaned = raw.strip()
            if cleaned.startswith("```json"):
                cleaned = cleaned[7:]
            ...
            reviewer = data.get("reviewer", "groq-vision")
    ...
    def review(self, request: VisualReviewRequest) -> VisualReviewResult:
        ...
        try:
            ...
            if self.__client is None:
                self.__client = create_groq_client(api_key)
            self.__api_key = None
            ...
            response_format = {
                ...
                "json_schema": {
                    ...
                    "schema": {
                        "type": "object",
                        "properties": {
                            "action": {"type": "string", "enum": ["pass", "warn", "human_review"]},
                            ...
                            "issues": {
                                ...
                                "items": {
                                    "type": "object",
                                    "properties": {
                                        "category": {"type": "string", "enum": ["layout", "responsive", "typography", "spacing", "visual_hierarchy", "image", "cta", "brand_consistency", "other"]},
                                        "severity": {"type": "string", "enum": ["info", "warning", "major"]},
                                        "viewport": {"type": ["string", "null"], "enum": ["desktop", "mobile", "cross_viewport", "null"]},
                        ...
                        "required": ["action", "summary", "issues", "reviewed_viewports"],
                        "additionalProperties": False
                    }
                }
            }
            ...
            if usage is not None:
                result.input_tokens = self._normalize_token(getattr(usage, "prompt_tokens", None))
                result.output_tokens = self._normalize_token(getattr(usage, "completion_tokens", None))
                result.total_tokens = self._normalize_token(getattr(usage, "total_tokens", None))
            else:
                result.input_tokens = result.output_tokens = result.total_tokens = None
            if not result.success:
                result.error = ProviderFailure(ErrorCode.INVALID_RESPONSE).safe_summary
            return result
...
def create_visual_quality_provider(config) -> VisualQualityProvider:
    ...
    if provider_type == "openai":
        ...
        return GroqVisualQualityProvider(api_key)
    else:
